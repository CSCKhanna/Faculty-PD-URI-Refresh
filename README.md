...
The updater fetches every enabled source listed in `data/sources.json`, writes verification snapshots and source-content hashes into `data/trainings.json`, and automatically publishes new opportunities from configured catalogs as `status: discovered`. It also checks every distinct training `sourceUrl` each night. A confirmed 404 or 410 must occur on two consecutive runs before an item is marked `source-removed` and hidden, which prevents a temporary outage from deleting a valid opportunity.

Sources with `catalogSync` are compared title-by-title against the dashboard. New catalog entries publish automatically, missing titles require two consecutive successful catalog checks before removal, and removed titles are restored automatically if they return. URI-ATL, NCFDD's public events archive, CIRTL's structured event feed, and the CUR community calendar have managed catalog rules. NCFDD Writing Challenge sessions are also read directly from the published schedule. Catalog minimums stop reconciliation when an extraction is unexpectedly sparse, so a layout change cannot masquerade as mass removals. Other providers still receive daily page-change, source-link, and expiration checks where their public pages do not expose a reliable catalog.

The NCFDD Faculty Success Program is permanently excluded by exact provider and title, including from future catalog discoveries. The updater also removes the disallowed URI sponsorship sentence from existing records and again after new discoveries are merged.
